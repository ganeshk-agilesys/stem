class IndustryController < ApplicationController
  def show
    response = open URI.escape("http://23.21.178.16/api/v1/industries/#{params[:id]}.json")
    ...
    if @industry.has_key?("sub_industry_ids") && !@industry["sub_industry_ids"].nil?
      ...
      @industry["sub_industry_ids"].each do |si|
        sub_response = JSON.parse(open(URI.escape("http://23.21.178.16/api/v1/industries/#{si}.json")).read)
